...
def states_to_wordclouds(hmm, word_map, syllable_dict, max_words=50, show=True):
    # Initialize.
    M = 200000
    n_states = len(hmm.A)
    wordclouds = []
    ...
    emission, states = hmm.generate_emission(M, word_map, syllable_dict)
    ...
    for i in range(n_states):
        obs_lst = obs_count[i]
        sentence = [word_map[j] for j in obs_lst]
...
def animate_emission(hmm, word_map, syllable_dict, M=8, height=12, width=12, delay=1):
    # Parameters.
    lim = 1200
    text_x_offset = 40
    text_y_offset = 80
    ...
    # Initialize.
    n_states = len(hmm.A)
    wordclouds = states_to_wordclouds(hmm, word_map, syllable_dict, max_words=20, show=False)
